...
// Credit helpers
db.getUserCredits = function(userId) {
  return db.prepare('SELECT * FROM user_credits WHERE user_id = ?').get(userId);
};

db.addUserCredits = function(userId, creditsToAdd) {
  const existing = db.getUserCredits(userId);
  if (existing) {
    return db.prepare(`
      UPDATE user_credits SET 
        ...
        updated_at = CURRENT_TIMESTAMP
      WHERE user_id = ?
    `).run(creditsToAdd, creditsToAdd, userId);
  } else {
    return db.prepare(`
      INSERT INTO user_credits (user_id, credits_remaining, credits_total, last_reset_date)
      VALUES (?, ?, ?, CURRENT_TIMESTAMP)
    `).run(userId, creditsToAdd, creditsToAdd);
  }
};

db.useUserCredits = function(userId, creditsToUse) {
  return db.prepare(`
    UPDATE user_credits SET 
      credits_remaining = credits_remaining - ?,
      updated_at = CURRENT_TIMESTAMP
    WHERE user_id = ? AND credits_remaining >= ?
  `).run(creditsToUse, userId, creditsToUse);
};

db.hasEnoughCredits = function(userId, requiredCredits) {
  const credits = db.getUserCredits(userId);
  return credits && credits.credits_remaining >= requiredCredits;
};
